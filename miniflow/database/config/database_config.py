...
class DatabaseConfig:
    ...
    Birden fazla veritabanı tipini destekler. Bağlantı parametreleri, DB-tipi özel `connect_args` birleştirmesi ve 
    genel engine ayarlarını tek bir konfigürasyonda toplar.
    """
    ...
    def __post_init__(self):
        ...
        if self.port is not None:
            try:
                ...
                raise DatabaseConfigurationError(field_name="port")
            
        # Credentials validation for DBs that require them (SQLite hariç)
        if self.db_type.requires_credentials():
            if not self.username:
                raise DatabaseConfigurationError(field_name="username")
            if self.password is None:
                raise DatabaseConfigurationError(field_name="password")
        
        # Non-SQLite validations for host, db_name, port
        if self.db_type != DatabaseType.SQLITE:
            if not self.host:
                raise DatabaseConfigurationError(field_name="host")
            if not self.db_name:
                raise DatabaseConfigurationError(field_name="db_name")
        ...
        if self.db_type == DatabaseType.SQLITE:
            if not self.sqlite_path or not self.sqlite_path.strip():
                raise DatabaseConfigurationError(field_name="sqlite_path")
        ...
        if self.db_type == DatabaseType.POSTGRESQL and self.statement_timeout_ms is not None:
            try:
                ...
                raise DatabaseConfigurationError(field_name="statement_timeout_ms")
            if timeout_ms < 0:
                raise DatabaseConfigurationError(field_name="statement_timeout_ms")
